...
class CoordinatesProvider(BaseProvider):
    ...
    def _preprocess(self, params: Dict[str, Any], gm: Any = None, init = False, **kwargs):
        ...
        action = memory.get_recent_history('actions')[-1]
        success = memory.get_recent_history('success')[-1]

        if init:
            raw_coordinates = params.get('raw_coordinates', {})
        else:
            raw_coordinates = memory.get_recent_history('raw_coordinates')[-1]
        ...
        if init or not is_valid_value(action):
            ...
            last_success_try_place_action = memory.get_recent_history('last_success_try_place_action')[-1]

            if skill_name is not None and "confirm_placement" in skill_name and success:
                actions = memory.get_recent_history('actions', config.max_recent_steps)
