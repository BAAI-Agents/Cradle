...
class SkillRegistry():
    ...
    def __init__(self,
        ...
        self.skill_from_default = skill_from_default
        self.skill_mode = skill_mode
        self.skill_names_basic = skill_names_basic
        self.skill_names_allow = skill_names_allow
        self.skill_names_deny = skill_names_deny
        self.skill_names_movement = skill_names_movement
        self.skill_names_map = skill_names_map
        self.skill_names_trade = skill_names_trade
        self.recent_skills = []
    ...
    def load_skills_from_file(self, file_path) -> Dict[str, Skill]:
        ...
        for skill_name in skill_local.keys():
            ...
            if not is_valid_value(skill_embedding): # The skill_embedding is invalid
                regenerate_flag = True
            if skill_name not in self.skill_registried.keys(): # The skill is not in the skill registry
                regenerate_flag = True

            if not regenerate_flag:
                logger.write(f"No need to regenerate skill {skill_name}")
                skills[skill_name] = Skill(skill_name,
                                           self.skill_registried[skill_name].skill_function,
    ...
    def load_skills_from_scripts(self) -> Dict[str, Skill]:
        ...
        skills = {}
        for skill_name in self.skill_registried.keys():

            skill_embedding = self.skill_registried[skill_name].skill_embedding
            skill_code_base64 = base64.b64encode(self.skill_registried[skill_name].skill_code.encode('utf-8')).decode('utf-8')

            regenerate_flag = False

            if skill_code_base64 != self.skill_registried[skill_name].skill_code_base64: # The skill_code is modified
                regenerate_flag = True
            if not is_valid_value(skill_embedding): # The skill_embedding is invalid
                regenerate_flag = True
            ...
            if not regenerate_flag:
                ...
                skills[skill_name] = Skill(skill_name,
                                           self.skill_registried[skill_name].skill_function,
                                           self.skill_registried[skill_name].skill_embedding,
                                           self.skill_registried[skill_name].skill_code,
                                           skill_code_base64)
            else: # skill_code has been modified, we should recompute embedding
                logger.write(f"Regenerate skill {skill_name}")
                skills[skill_name] = Skill(skill_name,
                                           self.skill_registried[skill_name].skill_function,
                                           self.get_embedding(skill_name, inspect.getdoc(self.skill_registried[skill_name].skill_function)),
                                           self.skill_registried[skill_name].skill_code,
                                           skill_code_base64)
