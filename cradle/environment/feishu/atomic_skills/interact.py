...
@register_skill("open_the_user_profile_menu")
def open_the_user_profile_menu():
    """
    This function can be used to open the user profile menu to set the status.
    """

    io_env.key_press("ctrl, shift, y")
    post_skill_wait(config.DEFAULT_POST_ACTION_WAIT_TIME)


@register_skill("click_attachment_button")
def click_attachment_button():
    """
    Click the Attachment button in a chat session.
    """
    button_x, button_y = 1742/config.DEFAULT_ENV_RESOLUTION[0], 1016/config.DEFAULT_ENV_RESOLUTION[1]  # Fixed coordinates of the button in the default resolution.
    click_at_position(button_x, button_y, io_env.LEFT_MOUSE_BUTTON)
