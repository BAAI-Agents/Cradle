...
def go_through_door(door):
    ...
    """
    go_to_icon(door, iterations=DEFAULT_GO_THROUGH_DOOR_ITERATIONS, terminal_threshold=DEFAULT_GO_THROUGH_DOOR_TERMINAL_THRESHOLD, debug=False, gm=None, character=["up", "down", "left", "right"])
...
def go_to_icon(target: str = "store", iterations=None, terminal_threshold=None, debug: bool = False, gm=None, character=None):
    ...
    """
    if not os.path.exists(f'./res/{config.env_sub_path}/icons/{target}.jpg'):
        return
...
def cv_go_to_icon(
    ...
    for step in range(iterations):
        ...
        # gm.unpause_game()
        # cur_screen_shot_path, cur_screen_shot_path_augmented = gm.capture_screen(draw_axis=config.draw_axis,
        #                                                                               draw_color_band=config.draw_color_band)

        cur_screen_shot_path = take_screenshot()
        # if the character is in the center of the cur_screen_shot, then the dis_x, dis_y is the distance between character and template,
        # otherwise, you can only use the info["bounding_box"] for the position of the character
        dis_xy, info = match_template(cur_screen_shot_path, template_file, debug=debug, character=character)
